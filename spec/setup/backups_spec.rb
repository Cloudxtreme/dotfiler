...
RSpec.describe Backup do
  ...
  def get_backup(local_tasks, app_tasks, enabled_tasks, disabled_tasks)
    ...
    backup = Backup.new '/backup/dir', host_info, io, store_factory
  end
  ...
  it 'should handle enable_tasks' do
    assert_enable_tasks({enabled: [], disabled: []}, [], {enabled: [], disabled: []})
    assert_enable_tasks({enabled: [], disabled: []}, ['task1', 'task2'], {enabled: [], disabled: []})
    assert_enable_tasks({enabled: [], disabled: []}, ['a'], {enabled: ['a'], disabled: []})
    assert_enable_tasks({enabled: [], disabled: []}, ['A', 'b2'], {enabled: ['a', 'b2'], disabled: []})
    assert_enable_tasks({enabled: ['a'], disabled: []}, ['A', 'b2'], {enabled: ['a', 'b2'], disabled: []})
    assert_enable_tasks({enabled: ['a'], disabled: ['b2', 'c']}, ['A', 'b2'], {enabled: ['a', 'b2'], disabled: ['c']})
  end
  
  it 'should handle disable_tasks' do
    assert_disable_tasks({enabled: [], disabled: []}, [], {enabled: [], disabled: []})
    assert_disable_tasks({enabled: [], disabled: []}, ['task1', 'task2'], {enabled: [], disabled: []})
    assert_disable_tasks({enabled: [], disabled: []}, ['a'], {enabled: [], disabled: ['a']})
    assert_disable_tasks({enabled: [], disabled: []}, ['A', 'b2'], {enabled: [], disabled: ['a', 'b2']})
    assert_disable_tasks({enabled: ['a'], disabled: []}, ['A', 'b2'], {enabled: [], disabled: ['a', 'b2']})
    assert_disable_tasks({enabled: ['a', 'c'], disabled: ['b2']}, ['A', 'b2'], {enabled: ['c'], disabled: ['b2', 'a']})
  end
...
RSpec.describe BackupManager do
  ...
  describe 'new_backup_tasks' do
    it 'should return new tasks per backup' do
      expect(manager_store).to receive(:fetch).with('backups', []).and_return ['/backup/dir1', 'backup/dir2']
      expect(Backup).to receive(:new).with('/backup/dir1', host_info, io, store_factory).and_return backup_store1
      expect(Backup).to receive(:new).with('/backup/dir2', host_info, io, store_factory).and_return backup_store2
      
      # TODO: add the remainder of the test
      backup_manager.new_backup_tasks []
      pending
    end
  end

...
module Dotfiler
  module Tasks
    ...
    #   Also, {Cli::Package#discover} will not list packages which return the +:no_sources+ status.
    #
    # * {#cleanup!} what should be cleaned up when a task is run.
    #   With this you can detect files that a Task leaves around and clean them up automatically,
    class Task
      extend Platform
      extend Forwardable
      include Platform
      ...
      def data?
        should_execute && (children? ? (any? { |sync_item| sync_item.status.kind != :no_sources }) : status.kind != :no_sources)
      end
